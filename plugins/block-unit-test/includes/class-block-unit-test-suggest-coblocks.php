...
class Block_Unit_Test_Suggest_CoBlocks {
	...
	public function notice() {
		...
		if ( $this->has_coblocks ) {
			...
			$link = '<a href="' . $url . '">' . esc_html__( 'activate CoBlocks &rarr;', 'block-unit-test' ) . '</a>';
		} else {
			$url  = esc_url( wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=coblocks' ), 'install-plugin_coblocks' ) );
			$link = '<a href="' . $url . '">' . esc_html__( 'install CoBlocks &rarr;', 'block-unit-test' ) . '</a>';
		}

		$coblocks = '<a href="https://coblocks.com" target="_blank">CoBlocks</a>';

		echo '<div data-dismissible="dismiss-coblocks-21" class="notice notice-info is-dismissible"><p>Block Unit Test' . wp_kses( sprintf( __( ' suggests performing a unit test for %1$s. To run the additional unit test, %2$s', 'block-unit-test' ), $coblocks, $link ), $allowed_html_array ) . '</p></div>';
	}
}
